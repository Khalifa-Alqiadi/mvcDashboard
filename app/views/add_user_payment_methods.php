...
<div class="card mb-4">
  <h5 class="card-header">انشاء طريقة دفع جديد</h5>
  <form class="card-body" action="/save_UserPayement" method="POST">
    ...
    <div class="row g-3">
      ...
      <div class="col-md-6">
        ...
        <select name="payement_id" class="form-select">
            <option value="">اختار طريقة الدفع</option>
            <?php
            foreach($params['payement'] as $payement){
                echo '<option value="'. $payement['id'] .'">'. $payement['name'] .'</option>';
            }
            ?>
        </select>
      </div>
